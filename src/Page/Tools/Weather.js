...
class Weather extends Component {
  ...
  componentDidMount () {
    ...
    if (this.props.myLocation && this.props.myLocation.adcode) {
      this.setState({
        location: this.props.myLocation
      })
      this.getWeather(this.props.myLocation)
    } else {
      getCurLocation((res) => {
        console.log(res)
        this.setState({
  ...
  render () {
    ...
    return (
      <div className="flex-c flex-c-b main Weather">
        <BackBar title="天气查询" noborder bgcolor="rgba(0, 0, 0, 0)" color="#fff" />
